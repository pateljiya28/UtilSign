...
export default function MainLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="min-h-screen bg-slate-950">
            {/* ── Header ─────────────────────────────────────────── */}
            <header className="bg-slate-900 border-b border-slate-800/60 sticky top-0 z-50">
                <div className="max-w-[1400px] mx-auto px-6 h-14 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        <ThemeToggle />
                        <button className="p-2 rounded-md text-slate-400 hover:text-white hover:bg-slate-800/60 transition-colors">
                        ...
                        <div className="relative" ref={profileRef}>
                            ...
                            {profileOpen && (
                                <div className="absolute right-0 mt-2 w-56 rounded-xl bg-slate-900 border border-slate-800/60 shadow-lg py-2 animate-fade-in z-50">
                                    <div className="px-4 py-2.5 border-b border-slate-800/40">
                                        <p className="text-[11px] text-slate-500 uppercase tracking-wider font-semibold">Signed in as</p>
                                        <p className="text-sm text-white truncate mt-0.5 font-medium">{userEmail}</p>
                                    </div>
                                    <button
