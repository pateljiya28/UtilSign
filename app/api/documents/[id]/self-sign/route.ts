...
export async function POST(
    ...
    try {
        ...
        // Fire audit log in parallel (non-blocking)
        const sigAuditPromise = logEvent({
            documentId: doc.id,
            signerId: signer.id,
        ...
        // Wait for the signature audit log to finish alongside the burn
        const [burnedBytes] = await Promise.all([
            burnSignaturesIntoPDF(pdfBytes, burnInputs),
            sigAuditPromise,
        ])
        ...
        // ── Mark completed + audit in parallel ─────────────────────────────────
        await Promise.allSettled([
            admin.from('documents').update({ status: 'completed' }).eq('id', doc.id),
            logEvent({
                documentId: doc.id,
                signerId: signer.id,
                actorEmail: user.email!,
